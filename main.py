...
# Fonction pour la méthode Chain Ladder
...
def mack_chain_ladder_model():
    ...
    for j in range(9):
        ...
        sigma.append(math.sqrt(s / (9-j)))

    # Compléter le Triangle
    for i, col in enumerate(Triangle.columns[1:]):
        for j in range(i+1):
            Triangle[col].at[2021-j] = Triangle[str(int(col)-1)].at[2021-j] + (sigma[j] * math.sqrt(Triangle[str(int(col)-1)].at[2021-j]))
...
# Création des fenêtres dans l'application
def main():
    st.title("Choix du modèle")
    option = st.selectbox("Choisissez un modèle", ("Méthode Chain Ladder", "Modèle du Mack Chain Ladder"))

    if option == "Méthode Chain Ladder":
        chain_ladder_method()
    elif option == "Modèle du Mack Chain Ladder":
